...
def get_bottleneck_dataset(model, img_dir, img_size):
    ...
    for i in range(train_img_gen.samples):
        ...
        bottleneck_x_l.append(tf.convert_to_tensor(model.predict(x_i)))
        bottleneck_y_l.append(tf.convert_to_tensor(y_i))
        ######### Your code ends here #########
...
def retrain(image_dir):
    ...
    # Fill in the parts indicated by #FILL#. No additional lines are required.
    # import pdb; pdb.set_trace()
    # adim = bottleneck_train_ds.get_shape().as_list()
    
    for i in bottleneck_train_ds:
        # import pdb; pdb.set_trace()
        adim = i[0].get_shape().as_list()
        break
    
    
        
    input = tf.keras.layers.Input(shape=adim)
    retrain_layer = tf.keras.layers.Dense(3, activation="softmax")(input)
    retrain_model = tf.keras.Model(outputs=retrain_layer, inputs=[input])
    
    loss = tf.keras.losses.CategoricalCrossentropy(from_logits=False)
    metric = tf.keras.metrics.CategoricalAccuracy(name='train_accuracy')
